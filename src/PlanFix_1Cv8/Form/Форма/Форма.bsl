...
Процедура ВыполнитьПроизвольныйЗапрос(Команда)
	ОчиститьСообщения();
	ВыполнитьПроизвольныйЗапросНаСервере();
КонецПроцедуры

&НаСервере
Процедура ВыполнитьПроизвольныйЗапросНаСервере()
	JSON = "";
	ДопПараметрыЗапроса = "sourceId="+СлужебныеДанные.ИдентификаторИБ;
	
	Если ЗначениеЗаполнено(ПроизвольныйЗапросТело) Тогда
	ДопПараметрыЗапроса = ДопПараметрыЗапроса + "&"+ПроизвольныйЗапросТело;		
	КонецЕсли; 
	
	ВнешняяОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	РезультатВыполнения = ВнешняяОбработкаОбъект.ВыполнитьHTTPЗапрос(ПроизвольныйЗапросИмяМетода, ДопПараметрыЗапроса, ПроизвольныйЗапросТип);
	Если РезультатВыполнения.ДанныеJSON <> Неопределено Тогда
		JSON = РезультатВыполнения.ТекстJSON;
	КонецЕсли;
