...
Процедура ПрочитатьНастройкиСопоставленияДанных()
	...
	Если НастройкаСопоставленияДанных <> Неопределено Тогда
		Если НастройкаСопоставленияДанных.Свойство("ШаблоныКонтактаВПланфикс") Тогда
			ШаблоныКонтактаВПланфикс = НастройкаСопоставленияДанных.ШаблоныКонтактаВПланфикс;
		КонецЕсли;
...
Процедура ЗаписатьНастройкиСопоставленияДанных()
	НастройкиПодключения = Новый Структура();
	НастройкиПодключения.Вставить("ШаблоныКонтактаВПланфикс", ШаблоныКонтактаВПланфикс);
...
Процедура ПолучитьШаблоныКонтактовНаСервере()
	...
	Элементы.ШаблонКонтактаВПланфикс.СписокВыбора.Очистить();
	
	Если МассивШаблонов.Количество() <> 0 Тогда
		Для каждого Шаблон Из МассивШаблонов Цикл
			Элементы.ШаблонКонтактаВПланфикс.СписокВыбора.Добавить(Шаблон.id,Шаблон.title);
		КонецЦикла;
		Элементы.ШаблонКонтактаВПланфикс.СписокВыбора.Добавить(МассивШаблонов.Количество() - 1,"Нет выбран");
		ШаблоныКонтактаВПланфикс = МассивШаблонов[МассивШаблонов.Количество() - 1].id;
	Иначе 
		Элементы.ШаблонКонтактаВПланфикс.СписокВыбора.Добавить(0,"Нет загруженных шаблонов, нажмите загрузить список шаблонов");
		ШаблоныКонтактаВПланфикс = 0;
	КонецЕсли;
...
Процедура ОтправитьДанныеВПФНаСервере()
	РезультатЗапроса.Очистить();
	ВнешняяОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	РезультатВыполненияЗапроса = ВнешняяОбработкаОбъект.ВыполнитьHTTPЗапрос("contact/contacts", JSON);
КонецПроцедуры
