...
Процедура ПолеСовпаденияПриИмпортеКомпанийВ1СПриИзменении(Элемент)
	...
	Если Элементы.ДекорацияПП1.Видимость Тогда
		ПолучитьИЗаполнитьПользовательскиеПоляНаСервере();
	КонецЕсли; 
КонецПроцедуры

&НаКлиенте
Процедура ПолеСовпаденияПриИмпортеКонтактовВPlanfixПриИзменении(Элемент)
	Элементы.ПользовательскоеПолеPlanfix.Видимость = ПолеСовпаденияПриИмпортеКонтактовВPlanfix = "custom";
	
	Если Элементы.ПользовательскоеПолеPlanfix.Видимость Тогда
		ПолучитьИЗаполнитьПользовательскиеПоляНаСервере();
	КонецЕсли; 
КонецПроцедуры
...
Процедура ПрочитатьНастройкиПодключения()
	...
	Если НастройкиПодключения <> Неопределено Тогда
		...
		Иначе 
			РежимОтладки = Истина; // Пока по умолчанию включено
		КонецЕсли; 
	КонецЕсли; 
КонецПроцедуры
...
Процедура ПрочитатьНастройкиСопоставленияДанных()
	...
	Если НастройкаСопоставленияДанных_contacts <> Неопределено Тогда
		...
		КонецЕсли;
		Если НастройкаСопоставленияДанных_contacts.Свойство("ПользовательскоеПолеСовпаденияПриИмпортеВPlanfix") Тогда
			ПользовательскоеПолеСовпаденияПриИмпортеВPlanfix = НастройкаСопоставленияДанных_contacts.ПользовательскоеПолеСовпаденияПриИмпортеВPlanfix;
		КонецЕсли;
		Если НастройкаСопоставленияДанных_contacts.Свойство("ПолеСовпаденияПриИмпортеОбъектаВPlanfixПоУмолчанию") Тогда
			ПолеСовпаденияПриИмпортеОбъектаВPlanfixПоУмолчанию = НастройкаСопоставленияДанных_contacts.ПолеСовпаденияПриИмпортеОбъектаВPlanfixПоУмолчанию;
		Иначе 
			ПолеСовпаденияПриИмпортеОбъектаВPlanfixПоУмолчанию = "name";
		КонецЕсли;
...
Процедура ЗасписатьНастройкиПодключения()
	...
	НастройкиПодключения.Вставить("РежимОтладки", РежимОтладки);
...
Процедура ЗаписатьНастройкиСопоставленияДанных()
	...
	НастройкиПодключения.Вставить("ПользовательскоеПолеСовпаденияПриИмпортеВPlanfix", ПользовательскоеПолеСовпаденияПриИмпортеВPlanfix);
	НастройкиПодключения.Вставить("ПолеСовпаденияПриИмпортеОбъектаВPlanfixПоУмолчанию", ПолеСовпаденияПриИмпортеОбъектаВPlanfixПоУмолчанию);
...
Функция НастройкиЗаполненоКорректно()
	...
	Если СтрНайти(Строка(ПолеСовпаденияПриИмпортеКомпанийВ1С), "Пользователь") > 0 Тогда
		...
		Если ПолеСовпаденияПриИмпортеКомпанийВ1С = "ДваПользовательскихПоля" Тогда
	...
	Если ПолеСовпаденияПриИмпортеКонтактовВPlanfix = "custom" И Не ЗначениеЗаполнено(ПользовательскоеПолеСовпаденияПриИмпортеВPlanfix) Тогда
		ТекстСообщения = НСтр("ru='Не удалось сохранить настройки, не заполнено пользовательское поле для определения совадения контакта при импорте в Planfix'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);
		Возврат Ложь;	
	КонецЕсли;
...
Процедура ЗаполнитьСлужебнуюИнформацию()
	...
	СлужебнаяИнформация = СлужебнаяИнформация + "Версия платформы:" + СлужебныеДанные.ВерсияПлатформы + Символы.ПС;
...
Процедура ПолучитьИЗаполнитьПользовательскиеПоляНаСервере()
	...
	Элементы.ПользовательскоеПолеСовпаденияПриИмпортеВPlanfix.СписокВыбора.Очистить();
	...
	Если МассивПолей.Количество() <> 0 Тогда
		Для каждого Поле Из МассивПолей Цикл
			...
			Элементы.ПользовательскоеПолеСовпаденияПриИмпортеВPlanfix.СписокВыбора.Добавить(Поле.id, Поле.name + " ( тип поля: "+ Строка(ТипПоля)+")");
		КонецЦикла; 
	КонецЕсли;
