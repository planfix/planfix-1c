...
Процедура ПолучитьСписокКонтатковИзПланфиксНаСервере()
	JSON = "";
	ВнешняяОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	РезультатВыполнения = ВнешняяОбработкаОбъект.ВыполнитьHTTPЗапрос("contact/list", "offset=0&pageSize=1");
	Если РезультатВыполнения.ДанныеJSON <> Неопределено Тогда
		JSON = ЗначениеВСтрокуВнутр(РезультатВыполнения.ДанныеJSON.contacts);
	КонецЕсли;
КонецПроцедуры
