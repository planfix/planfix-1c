...
Процедура ПолучитьJSONКонтатковИзПланфиксНаСервере()
	JSON = "";
	ВнешняяОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	РезультатВыполнения = ВнешняяОбработкаОбъект.ВыполнитьHTTPЗапрос("contact/list", "offset=0&pageSize=" + КоличествоЗаписейВТестовомЗапросе + "&sourceId=" + СлужебныеДанные.ИдентификаторИБ);
...
Функция ПолучитьДанныеИзПланфиксНаСервере()
	...
	ВнешняяОбработкаОбъект = РеквизитФормыВЗначение("Объект");
	мРезультатЗапроса = ВнешняяОбработкаОбъект.ВыполнитьHTTPЗапрос("contact/list", "offset=0&pageSize=" + КоличествоЗаписейВТестовомЗапросе + "&sourceId=" + СлужебныеДанные.ИдентификаторИБ);
