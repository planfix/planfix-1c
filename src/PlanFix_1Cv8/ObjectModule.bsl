...
Функция ОтправитьДанныеВPlanfixВФоне()Экспорт
	...
	Попытка 
		...
		ЗаписатьСобытиеВЖурналРегистрации(НСтр("ru = 'Интеграция с Planfix. Выгрузка данных'", ОбщегоНазначения.КодОсновногоЯзыка()), 
...
Функция ПолучитьДанныеИзPlanfixВФоне(ТолькоИзмененные = Истина)Экспорт
	...
	Попытка 
		...
		ЗаписатьСобытиеВЖурналРегистрации(НСтр("ru = 'Интеграция с Planfix. Загрузка данных " + МетодПолученияДанных + "'", ОбщегоНазначения.КодОсновногоЯзыка()), 
...
Процедура ЗаписатьСобытиеВЖурналРегистрации(ИмяСобытия = Неопределено, ТекстСообщения, УровеньЖурнала = Неопределено)
	Если ИмяСобытия = Неопределено Тогда
		ИмяСобытия = НСтр("ru = 'Интеграция с Planfix. Обмен данными'", ОбщегоНазначения.КодОсновногоЯзыка());	
	КонецЕсли;
