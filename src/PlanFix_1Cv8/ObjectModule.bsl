...
Функция ВыполнитьОбменДаннымиСПланфикс(ТестовыйJSON = "") Экспорт
	...
	Для каждого  СтруктураОбъекта Из СтруктураМетаданных Цикл
		...
		СчетчикВыгружаемыхОбъектов = СчетчикВыгружаемыхОбъектов + РезультатВыполненияЗапроса.Выбрать().Количество(); 
	КонецЦикла; 
	
	Если СокрЛП(ТекстJSON) <> "" Тогда
		РезультатВыполнения = ОтправитьДанные(ТекстJSON, РезультатВыполнения);	
		РезультатВыполнения.РезультатВыгрузки.КоличествоЗаписей = СчетчикВыгружаемыхОбъектов;
	КонецЕсли; 
...
Функция ПолучитьДанные()
	РезультатВыполнения = ВыполнитьHTTPЗапрос("contact/list", "offset=0&pageSize=1");
	Если РезультатВыполнения.ДанныеJSON <> Неопределено Тогда
		Возврат ЗначениеВСтрокуВнутр(РезультатВыполнения.ДанныеJSON.contacts);
	КонецЕсли;
...
Функция ВыполнитьHTTPЗапрос(ИмяМетода, ТелоЗапроса = "", HTTPМетод = "POST") Экспорт
	...
	Иначе 
		РезультатВыполнения.ОписаниеОшибки = НСтр("ru='Ошибка! Не удалось установить соединение с сервером (" + HTTPОтвет.КодСостояния + ": " + HTTPОтвет.ПолучитьТелоКакСтроку() + ")'");
		Возврат РезультатВыполнения;
	КонецЕсли;
