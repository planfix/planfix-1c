...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	РегистрационныеДанные.Вставить("Наименование", "Модуль интеграции с ПланФикс");
	РегистрационныеДанные.Вставить("БезопасныйРежим", Ложь);
	РегистрационныеДанные.Вставить("Версия", "1.5.0");
	...
	ДобавитьКоманду(ТаблицаКоманд, "Открыть форму настроек обмена с ПланФикс", "ОктрытьФорму", "ОткрытиеФормы",  Ложь);
	ДобавитьКоманду(ТаблицаКоманд, "Выполнить обмен данными с ПланФикс", "ВыполнитьОбмен", "ВызовСерверногоМетода",  Истина);
	ДобавитьКоманду(ТаблицаКоманд, "Отправить данные в ПланФикс", "ОтправитьДанныеВПланФикс", "ВызовСерверногоМетода",  Истина);
	ДобавитьКоманду(ТаблицаКоманд, "Получить данные из ПланФикс", "ПолучитьДанныеИзПланФикс", "ВызовСерверногоМетода",  Истина);
...
Процедура ВыполнитьКоманду(ИдентификаторКоманды, ОбъектыНазначения) Экспорт
	...
	ИначеЕсли  ИдентификаторКоманды = "ОтправитьДанныеВПланФикс" Тогда 
		ОтправитьДанныеВПланФиксВФоне();
	ИначеЕсли ИдентификаторКоманды = "ПолучитьДанныеИзПланФикс" Тогда 
		ПолучитьДанныеИзПланФиксВФоне();
	КонецЕсли;	
...
Функция ВыполнитьОбменДаннымиВФоне()Экспорт
	...
	Попытка 
		...
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Интеграция с ПланФикс. Обмен данными'", ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация, , ,
		НСтр("ru = 'Результат обмена данными. " + ПолучитьТекстСообщенияРезультатаВыполнения(РезультатВыполнения)+ "'"));
	Исключение
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Интеграция с ПланФикс. Обмен данными'", ОбщегоНазначения.КодОсновногоЯзыка()),
...
Процедура ВыполнитьОбменДанными(РезультатВыполнения) 
	...
	Для каждого  СтруктураОбъектаМетаданных Из СтруктураМетаданных Цикл
		...
		ВыполнитьВыгрузкуДанныхРекурсивно(СтруктураОбъектаМетаданных, СлужебныеДанные.РазмерПорции, ПоследняяСсылка, РезультатВыполнения, НастройкаСопоставленияДанных);
		...
		Если СтруктураОбъектаМетаданных.Ключ = "contacts" Тогда
			...
			ВыполнитьЗагрузкуДанныхРекурсивно(СлужебныеДанные.ИдентификаторИБ, РезультатВыполнения,0, СлужебныеДанные.РазмерПорции, ИмяМетода, ДопПараметрыЗапроса);
...
Функция ОтправитьДанныеВПланФиксВФоне()Экспорт
	...
	Попытка 
		ОтправитьДанныеВПланФикс(РезультатВыполнения);
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Интеграция с ПланФикс. Выгрузка данных'", ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация, , ,
...
Процедура ОтправитьДанныеВПланФикс(РезультатВыполнения)
...
Функция ПолучитьДанныеИзПланФиксВФоне(ТолькоИзмененные = Истина)Экспорт
	...
	Попытка 
		ПолучитьДанныеИзПланФикс(ТолькоИзмененные, РезультатВыполнения);
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Интеграция с ПланФикс. Загрузка данных " + МетодПолученияДанных + "'", ОбщегоНазначения.КодОсновногоЯзыка()),
		УровеньЖурналаРегистрации.Информация, , ,
...
Процедура ПолучитьДанныеИзПланФикс(ТолькоИзмененные, РезультатВыполнения)
...
Функция ВыполнитьВыгрузкуДанныхРекурсивно(СтруктураОбъектаМетаданных, РазмерПорции, ПоследняяСсылка, РезультатВыполнения, НастройкаСопоставленияДанных)
	...
	РезультатВыполненияЗапроса = ПолучитьПорциюДанныхДляОбмена(СтруктураОбъектаМетаданных.Значение, РазмерПорции, ПоследняяСсылка, НастройкаСопоставленияДанных);
...
Функция СофрмироватьJSON(ТаблицаВыгружаемыхДанных, ИмяОбъектаПланфикс, ИдентификаторИБ, НастройкаСопоставленияДанных) Экспорт
	...
	Если ИмяОбъектаПланфикс = "contacts" ИЛИ ИмяОбъектаПланфикс = "contact_persons" Тогда
	...
	Для каждого СтрокаТЗ Из ТаблицаВыгружаемыхДанных Цикл
		...
		Для каждого КолонкаТЗ Из ТаблицаВыгружаемыхДанных.Колонки Цикл
			...
			ОпределитьИЗаполнитьОбщиеСвойстваОбъектаJSON(ЗаписьJSON, СтрокаТЗ, КолонкаТЗ, НастройкаСопоставленияДанных);
			...
			Если ТипЗнч(СтрокаТЗ[КолонкаТЗ.Имя]) = Тип("ТаблицаЗначений") Тогда
				ДозаполнитьJSONДаннымиИзВложеннойТаблицыЗначенийРекурсивно(ИмяОбъектаПланфикс,ЗаписьJSON, СтрокаТЗ[КолонкаТЗ.Имя],КолонкаТЗ.Имя,НастройкаСопоставленияДанных);
				Продолжить;	  
			КонецЕсли;
		КонецЦикла;
	...
	// Дополняем массив данными объектов с таким же типом данных в Планфикс 
...
Процедура ДозаполнитьJSONДаннымиИзВложеннойТаблицыЗначенийРекурсивно(ИмяОбъектаПланфикс, ЗаписьJSON, ТаблицаЗначений, ИмяТаблицы, НастройкаСопоставленияДанных)
...
Процедура ОпределитьИЗаполнитьОбщиеСвойстваОбъектаJSON(ЗаписьJSON, СтрокаТЗ, КолонкаТЗ, НастройкаСопоставленияДанных)
	// Для каждого типа объекта тут прийдется по своему заполнять 
	...
	КонтактыОпределитьИЗаполнитьОбщиеСвойстваОбъектаJSON(ЗаписьJSON, СтрокаТЗ, КолонкаТЗ, НастройкаСопоставленияДанных);
...
Процедура КонтактыОпределитьИЗаполнитьОбщиеСвойстваОбъектаJSON(ЗаписьJSON, СтрокаТЗ, КолонкаТЗ, НастройкаСопоставленияДанных)
	
	// Для контактных лиц к идентификатору объекта добавляем имя объекта, для контрагентов добавляем к версии объекта, версию объекта партнера	
	Если КолонкаТЗ.Имя = "Ссылка" Тогда
		Если ТипЗнч(СтрокаТЗ.Ссылка) = ПолучитьТипСправочникаКонтактныхЛиц()Тогда
			...
			// Выгружаем идентификатор владельца (партнер/контрагент). В Планфикс контакты пока есть только у компаний.
			// УТКА
			Если СтрокаТЗ.Ссылка.Владелец <> Неопределено Тогда
		...
		Если ТипЗнч(СтрокаТЗ.Ссылка) = Тип("СправочникСсылка.Контрагенты") И СтрокаТЗ.Ссылка.Метаданные().Реквизиты.Найти("Партнер") <> Неопределено Тогда
			ВерсияДанных = СокрЛП(СтрокаТЗ.Ссылка.ВерсияДанных) + " " + СокрЛП(СтрокаТЗ.Ссылка.Партнер.ВерсияДанных);
		Иначе 
	...
	Если КолонкаТЗ.Имя = "КонтактнаяИнформация" И ТипЗнч(СтрокаТЗ[КолонкаТЗ.Имя]) = Тип("ТаблицаЗначений")  Тогда
		...
		Если МассивПоиска.Количество() > 0 Тогда
			...
			Для каждого СтрокаТелефон Из МассивПоиска  Цикл
				...
				ЗаписьJSON.ЗаписатьЗначение(НормализоватьНомерТелефона(СтрокаТелефон.НомерТелефона));
...
Функция ВыполнитьЗагрузкуДанныхРекурсивно(ИдентификаторИБ, РезультатВыполнения, Смещение = 0, РазмерПорции = 100, ИмяМетода, ДопПараметрыЗапроса)
	
	ОтветТекстJSON = ПолучитьДанные(ИмяМетода, Смещение, РазмерПорции, ДопПараметрыЗапроса, РезультатВыполнения);
	...
	ВыполнитьЗагрузкуДанныхРекурсивно(ИдентификаторИБ, РезультатВыполнения, РезультатВыполнения.РезультатЗагрузки.КоличествоЗаписейСмещение, РазмерПорции, ИмяМетода, ДопПараметрыЗапроса)
	
КонецФункции

Функция ПолучитьДанные(ИмяМетода, Смещение, РазмерПорции, ДопПараметрыЗапроса, РезультатВыполнения)
...
Функция ОбработатьИЗагрузитьДанные(ТекстJSON, ИдентификаторИБ, РезультатВыполнения) Экспорт
	...
	Для каждого ЭлементДанныхJSON Из СтруктураДанныхJSON Цикл
		...
		Для каждого ОбъектJSON Из ЭлементДанныхJSON.Значение Цикл
			...
			УстановитьЗначениеСвойствОбъекта(НастройкаСопоставленияДанных, ТекущийОбъект, ОбъектJSON);
...
Функция ПолучитьОбъект(НастройкаСопоставленияДанных, МенеджерОбъекта, ОбъектJSON)
...
Функция ПолучитьОбъектПоПравиламИмпорта(НастройкаСопоставленияДанных, МенеджерОбъекта, ОбъектJSON)
	...
	Если НастройкаСопоставленияДанных.ПолеСовпаденияПриИмпортеКонтактовВ1С <> "" И НастройкаСопоставленияДанных.ПолеСовпаденияПриИмпортеКомпанийВ1С <> ""
		 ...
		 Если ПолеСовпаденияОбъекта = "Наименование" И ЗначениеЗаполнено(ОбъектJSON.name)Тогда
			 ОбъектСсылка =  МенеджерОбъекта.НайтиПоНаименованию(ПолучитьНаименованиеОбъектаИзJSON(ОбъектJSON));
		 ...
		 ИначеЕсли ПолеСовпаденияОбъекта = "ДваПользовательскихПоля" Тогда
		 ...
		 ИначеЕсли ПолеСовпаденияОбъекта = "АдресЭлектроннойПочты" И ЗначениеЗаполнено(ОбъектJSON.email) Тогда
			 ОбъектСсылка = ПолучитьСсылкуНаОбъектПоEmail(МенеджерОбъекта, ОбъектJSON.email); 
		 ИначеЕсли ПолеСовпаденияОбъекта = "Телефон" И ЗначениеЗаполнено(ОбъектJSON.phones) Тогда
			 ОбъектСсылка = ПолучитьСсылкуНаОбъектПоНомеруТелефона(МенеджерОбъекта, ОбъектJSON.phones); 
		 КонецЕсли;
...
Функция ПолучитьСсылкуНаОбъектПоДвумПользовательскимПолям(НастройкаСопоставленияДанных, МенеджерОбъекта, ОбъектJSON)
	...
	Если ЗначениеЗаполнено(ИдентификаторПоля1) И ЗначениеПоля1 <> Неопределено И ЗначениеПоля1 <> ""
		...
		ИмяРеквизитаОбъекта1 = НастройкаСопоставленияДанных.ТаблицаСопоставленияПользовательскихПолейИРеквизитов.Найти(ИдентификаторПоля1,"ИдентификаторПоляВПланфикс").ИмяРеквизитаОбъекта;
		ИмяРеквизитаОбъекта2 = НастройкаСопоставленияДанных.ТаблицаСопоставленияПользовательскихПолейИРеквизитов.Найти(ИдентификаторПоля2,"ИдентификаторПоляВПланфикс").ИмяРеквизитаОбъекта;
...
Функция ПолучитьСсылкуНаОбъектПоEmail(МенеджерОбъекта, АдресЭлектроннойПочты)
...
Функция ПолучитьСсылкуНаОбъектПоНомеруТелефона(МенеджерОбъекта, МассивНомеров)
...
Процедура УстановитьЗначениеСвойствОбъекта(НастройкаСопоставленияДанных, ТекущийОбъект, ОбъектJSON)
	...
	ИначеЕсли ТекущийОбъект.Метаданные().Имя = "Контрагенты" Тогда 
		...
		Если ТекущийОбъект.Ссылка.Пустая() Тогда
			 УстановитьЗначениеСвойствНовыйКонтрагент(НастройкаСопоставленияДанных, ТекущийОбъект, ОбъектJSON);
		Иначе
			 УстановитьЗначениеСвойствТекущийКонтрагент(НастройкаСопоставленияДанных, ТекущийОбъект, ОбъектJSON);
		КонецЕсли; 
...
Процедура УстановитьЗначениеСвойствТекущийКонтрагентУТКА(НастройкаСопоставленияДанных, Контрагент, КонтактJSON)
	...
	// Пока не обновляем данные партнера, ждем кейсов
	//УстановитьЗначениеСвойствТекущийПартнер(Контрагент.Партнер.ПолучитьОбъект(),КонтактJSON);
	Контрагент.Записать();
...
Функция  УстановитьЗначениеСвойствНовыйПартнер(НастройкаСопоставленияДанных, НовыйПартнер, КонтактJSON)
	...
	Если ИнформацияПоКонтактнымЛицам <> Неопределено Тогда
...
Процедура УстановитьЗначениеСвойствТекущийПартнер(НастройкаСопоставленияДанных, Партнер, КонтактJSON)
	...
	Для каждого Поле Из КонтактJSON Цикл
		...
		ИначеЕсли Поле.Ключ = "contacts" Тогда
			СоздатьИзменитьКонтактныеЛицаПартнера(НастройкаСопоставленияДанных, Партнер, Поле.Значение);
		КонецЕсли;
	КонецЦикла;
...
Процедура СоздатьИзменитьКонтактныеЛицаПартнера(НастройкаСопоставленияДанных, Партнер, МассивКонтактныхЛиц)
	...
	Для каждого КонтактJSON Из  МассивКонтактныхЛиц Цикл
		...
		// пробуем по идентифкатору найти
		Если КонтактJSON.Свойство("sourceObjectId") И ЗначениеЗаполнено(КонтактJSON.sourceObjectId) Тогда
			Отбор = Новый Структура();
			Отбор.Вставить("Ссылка", Справочники.КонтактныеЛицаПартнеров.ПолучитьСсылку(ПолучитьGUID(КонтактJSON)));
		...
		Если ОбъектКЛ = Неопределено Тогда
			
			// если не нашли ищем по наименованию
			Отбор = Новый Структура();
			Отбор.Вставить("Наименование", ПолучитьНаименованиеОбъектаИзJSON(КонтактJSON));
			МассивИскомыхСтрок = ТаблицаКЛ.НайтиСтроки(Отбор);
			Если МассивИскомыхСтрок.Количество() = 0 Тогда
...
Процедура УстановитьЗначениеСвойствНовыйКонтрагент(НастройкаСопоставленияДанных, НовыйКонтрагент, КонтактJSON)
	ИнформацияПоКонтактнымЛицам = Неопределено;
	...
	Для каждого Поле Из КонтактJSON Цикл
		...
		ИначеЕсли Поле.Ключ = "groupName" Тогда
			НовыйКонтрагент.Покупатель = (СтрНайти(Поле.Значение, "Клиент") <> 0 
											ИЛИ СтрНайти(Поле.Значение, "Покупатель") <> 0);
			НовыйКонтрагент.Поставщик = СтрНайти(Поле.Значение, "Поставщик") <> 0;
			НовыйКонтрагент.ПрочиеОтношения = (СтрНайти(Поле.Значение, "Подрядчик") <> 0 
												ИЛИ СтрНайти(Поле.Значение, "Партнер") <> 0 
	...
	УстановитьКонтактнуюИнформациюКонтрагента(НастройкаСопоставленияДанных, НовыйКонтрагент, КонтактJSON);
	...
	Если ИнформацияПоКонтактнымЛицам <> Неопределено Тогда
		СоздатьКонтактныеЛица(НастройкаСопоставленияДанных, НовыйКонтрагент.Ссылка, ИнформацияПоКонтактнымЛицам);
	Иначе
		СоздатьКонтактноеЛицо(НастройкаСопоставленияДанных, НовыйКонтрагент.Ссылка, КонтактJSON);	
	КонецЕсли; 
КонецПроцедуры

Процедура УстановитьЗначениеСвойствТекущийКонтрагент(НастройкаСопоставленияДанных, Контрагент, КонтактJSON)
	ИнформацияПоКонтактнымЛицам = Неопределено;
	...
	УстановитьКонтактнуюИнформациюКонтрагента(НастройкаСопоставленияДанных, Контрагент, КонтактJSON);
	...
	Если Контрагент.КонтактноеЛицо.Пустая()Тогда
		Если ИнформацияПоКонтактнымЛицам <> Неопределено Тогда
			СоздатьКонтактныеЛица(НастройкаСопоставленияДанных, Контрагент.Ссылка, ИнформацияПоКонтактнымЛицам);
		Иначе
			СоздатьКонтактноеЛицо(НастройкаСопоставленияДанных, Контрагент.Ссылка, КонтактJSON);	
		КонецЕсли;
	Иначе 
		Если ИнформацияПоКонтактнымЛицам <> Неопределено Тогда
			ИзменитьКонтактныеЛица(НастройкаСопоставленияДанных, Контрагент, ИнформацияПоКонтактнымЛицам);
		Иначе
			ИзменитьКонтактноеЛицо(НастройкаСопоставленияДанных, Контрагент.КонтактноеЛицо, Контрагент.Ссылка, КонтактJSON);
		КонецЕсли; 
	КонецЕсли; 
КонецПроцедуры

Процедура УстановитьКонтактнуюИнформациюКонтрагента(НастройкаСопоставленияДанных, Контрагент, КонтактJSON)
...
Процедура СоздатьКонтактныеЛица(НастройкаСопоставленияДанных, КонтрагентКомпания, МассивКонтактныхЛиц)
	
	// Ищем или создаем контрагента, из него создаем контактное лицо и связываем с компанией 
	...
	Для каждого КонтактJSON Из  МассивКонтактныхЛиц Цикл
		...
		УстановитьЗначениеСвойствОбъекта(НастройкаСопоставленияДанных, КонтрагентДляКЛ, КонтактJSON);
		...
		УстановитьКонтактнуюИнформациюКонтактногоЛица(НастройкаСопоставленияДанных, КонтактJSON, КонтрагентДляКЛ, КонтактноеЛицоОбъект);
		...
		КонтрагентОбъект = КонтрагентКомпания.ПолучитьОбъект();
		КонтрагентОбъект.КонтактноеЛицо = КонтактноеЛицоОбъект.Ссылка;
		КонтрагентОбъект.Записать();
	КонецЦикла; 
КонецПроцедуры

Процедура ИзменитьКонтактныеЛица(НастройкаСопоставленияДанных, КонтрагентКомпания, МассивКонтактныхЛиц)
...
Процедура СоздатьКонтактноеЛицо(НастройкаСопоставленияДанных, Контрагент, КонтактJSON)
	...
	УстановитьКонтактнуюИнформациюКонтактногоЛица(НастройкаСопоставленияДанных, КонтактJSON, Контрагент, КонтактноеЛицоОбъект);
...
Процедура ИзменитьКонтактноеЛицо(НастройкаСопоставленияДанных, КонтактСсылка, Контрагент, КонтактJSON)
...
Процедура УстановитьКонтактнуюИнформациюКонтактногоЛица(НастройкаСопоставленияДанных, КонтактJSON, Контрагент, КонтактноеЛицоОбъект)
...
Функция ВыполнитьHTTPЗапрос(ИмяМетода, ТелоЗапроса = "", HTTPМетод = "POST", Таймаут = 60) Экспорт
	...
	HTTPЗапрос = Новый HTTPЗапрос(СтруктураURI.ПутьНаСервере + ИмяМетода + "?access_token=" 
...
Функция ПолучитьНаименованиеОбъектаИзJSON(КонтактJSON)
	Если СлужебныеДанные.ИмяКонфигурации = "УправлениеТорговлей" 
		...
		Если  КонтактJSON.Свойство("isCompany") И Не КонтактJSON.isCompany Тогда
			Наименование = КонтактJSON.lastname + " " + КонтактJSON.name + " " + КонтактJSON.midname;
		Иначе 
	...
	ИначеЕсли СлужебныеДанные.ИмяКонфигурации = "УправлениеНебольшойФирмой" Тогда
		...
		// тут для контакта в программе Имя Фамиля указано, хотя правильнее было бы как и везеде ФИО, оставляем пока ФИО
		// контакт
		Если  КонтактJSON.Свойство("isCompany") И Не КонтактJSON.isCompany Тогда
			Наименование = КонтактJSON.lastname + " " + КонтактJSON.name + " " + КонтактJSON.midname;
		Иначе 
...
Процедура ЗаписатьСтруктуруНаименованияОбъектаВJSON(Ссылка, ЗаписьJSON)
	...
	Если СлужебныеДанные.ИмяКонфигурации = "УправлениеТорговлей" 
		...
		Если (ТипЗнч(Ссылка) = Тип("СправочникСсылка.Контрагенты") 
			...
			И Ссылка.ЮрФизЛицо = Перечисления.КомпанияЧастноеЛицо.ЧастноеЛицо Тогда 
...
Функция ПолучитьПорциюДанныхДляОбмена(СтруктураОбъектаМетаданных, КоличествоЭлементов, ПоследняяСсылка, НастройкаСопоставленияДанных) Экспорт
	...
	//ТекстЗапросаСоединениеСТаблицами = ПолучитьТекстЗапросаСоединенияПоИмениТаблицы(СтруктураОбъектаМетаданных.ИмяТаблицыБд);
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ПоследняяСсылка",ПоследняяСсылка);
	
	// УНФ текст запроса для справочника КонтактныеЛица
	Если  СтруктураОбъектаМетаданных.ИмяТаблицыБд = "Справочник.КонтактныеЛица" Тогда 
	...
	Иначе 
	  ...
	  //|   " + ТекстЗапросаСоединениеСТаблицами + "
		|ГДЕ
	...
	КонецЕсли;
	Запрос.Текст = УстановитьДополнительныеУсловияВыборкиПоИмениТаблицы(Запрос.Текст, СтруктураОбъектаМетаданных.ИмяТаблицыБд);
...
Функция ПолучитьТекстЗапросаСоединенияПоИмениТаблицы(ИмяТаблицыБд)
	ТекстЗапросаСоединениеСТаблицами = "";
	// Контактные лица только компаний
	Если ИмяТаблицыБд = "Справочник.КонтактныеЛица" Тогда  //УНФ
		ТекстЗапросаСоединениеСТаблицами = "ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты КАК Контрагенты
											|ПО ТаблицаОбъекта.Ссылка = Контрагенты.КонтактноеЛицо";
	КонецЕсли; 
	
	Возврат ТекстЗапросаСоединениеСТаблицами
		
КонецФункции
 
Функция УстановитьДополнительныеУсловияВыборкиПоИмениТаблицы(ТекстЗапроса, ИмяТаблицыБд);
	// Контактные лица только компаний
	...
	ИначеЕсли ИмяТаблицыБд = "Справочник.КонтактныеЛица" Тогда //УНФ
...
Функция ПолучитьНастройкуСопоставленияДанных(ИмяЗагружаемогоОбъектаПанфикс) Экспорт
	...
	Если ИмяКлючаНастроек = "contacts" Тогда
		...
		Если НастройкаСопоставленияДанных <> Неопределено И НастройкаСопоставленияДанных.Свойство("ШаблонКонтактаВПланфикс") Тогда
			СтруктураНастройкиОбъекта.Вставить("ШаблонКонтактаВПланфикс", НастройкаСопоставленияДанных.ШаблонКонтактаВПланфикс);	
		Иначе 
			СтруктураНастройкиОбъекта.Вставить("ШаблонКонтактаВПланфикс", "");	
		КонецЕсли;
		Если НастройкаСопоставленияДанных <> Неопределено И НастройкаСопоставленияДанных.Свойство("ШаблонКомпанииВПланфикс") Тогда
			СтруктураНастройкиОбъекта.Вставить("ШаблонКомпанииВПланфикс", НастройкаСопоставленияДанных.ШаблонКомпанииВПланфикс);	
		Иначе 
			СтруктураНастройкиОбъекта.Вставить("ШаблонКомпанииВПланфикс", "");	
		КонецЕсли;
		Если НастройкаСопоставленияДанных <> Неопределено И НастройкаСопоставленияДанных.Свойство("ОпцииИмпортаКонтактовВПланфикс") Тогда
			СтруктураНастройкиОбъекта.Вставить("ОпцииИмпортаКонтактовВПланфикс", НастройкаСопоставленияДанных.ОпцииИмпортаКонтактовВПланфикс);	
		Иначе 
			СтруктураНастройкиОбъекта.Вставить("ОпцииИмпортаКонтактовВПланфикс", "");	
		КонецЕсли;
		Если НастройкаСопоставленияДанных <> Неопределено И НастройкаСопоставленияДанных.Свойство("ПолеСовпаденияПриИмпортеКонтактовВПланфикс") Тогда
			СтруктураНастройкиОбъекта.Вставить("ПолеСовпаденияПриИмпортеКонтактовВПланфикс", НастройкаСопоставленияДанных.ПолеСовпаденияПриИмпортеКонтактовВПланфикс);	
		Иначе 
			СтруктураНастройкиОбъекта.Вставить("ПолеСовпаденияПриИмпортеКонтактовВПланфикс", "");	
		КонецЕсли;
...
Функция ПолучитьВидНомераТелефона(НастройкаСопоставленияДанных, ИдентификаторТипаПФ, ОбъектСсылка)
	...
	ИначеЕсли ОбъектСсылка.Метаданные().Имя = "КонтактныеЛицаПартнеров" Тогда
		...
		ИначеЕсли ИдентификаторТипаПФ = 2 Тогда 
	...
	ИначеЕсли ОбъектСсылка.Метаданные().Имя = "КонтактныеЛица" Тогда
		ВидНомераТелефонаПредопределенный = Справочники.ВидыКонтактнойИнформации.ТелефонКонтактногоЛица;
