...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	РегистрационныеДанные.Вставить("Версия", "1.4.0");
...
Функция ВыполнитьОбменДанными() Экспорт
	...
	Для каждого  СтруктураОбъектаМетаданных Из СтруктураМетаданных Цикл
		...
		Если СтруктураОбъектаМетаданных.Ключ = "contacts" Тогда
			...
			ВыполнитьЗагрузкуДанныхРекурсивно(СлужебныеДанные.ИдентификаторИБ, РезультатВыполнения,0, РазмерПорции, ИмяМетода, ДопПараметрыЗапроса);
...
Процедура КонтактыОпределитьИЗаполнитьОбщиеСвойстваОбъектаJSON(ЗаписьJSON, СтрокаТЗ, КолонкаТЗ)
	...
	Если КолонкаТЗ.Имя = "Ссылка" Тогда
		...
		Если ТипЗнч(СтрокаТЗ.Ссылка) = Тип("СправочникСсылка.КонтактныеЛицаПартнеров") Тогда
			УникальныйИдентификатор = "contact_persons_" + СокрЛП(СтрокаТЗ[КолонкаТЗ.Имя].УникальныйИдентификатор());
			ЗаписьJSON.ЗаписатьИмяСвойства("isCompany");
			ЗаписьJSON.ЗаписатьЗначение(Ложь);
		Иначе 
			УникальныйИдентификатор = СокрЛП(СтрокаТЗ[КолонкаТЗ.Имя].УникальныйИдентификатор());	
		КонецЕсли; 
		
		ЗаписьJSON.ЗаписатьИмяСвойства("sourceObjectId");
		ЗаписьJSON.ЗаписатьЗначение(УникальныйИдентификатор);	

		Если ТипЗнч(СтрокаТЗ.Ссылка) = Тип("СправочникСсылка.Контрагенты") Тогда
			ВерсияДанных = СокрЛП(СтрокаТЗ[КолонкаТЗ.Имя].ВерсияДанных) + " " + СокрЛП(СтрокаТЗ[КолонкаТЗ.Имя].Партнер.ВерсияДанных);
		Иначе 
			ВерсияДанных = СокрЛП(СтрокаТЗ[КолонкаТЗ.Имя].ВерсияДанных);	
		КонецЕсли;
	...
	Если КолонкаТЗ.Имя = "Наименование" И СокрЛП(СтрокаТЗ[КолонкаТЗ.Имя]) <> ""  
		И ТипЗнч(СтрокаТЗ.Ссылка) = Тип("СправочникСсылка.КонтактныеЛицаПартнеров") Тогда
...
Процедура СоздатьИзменитьКонтактныеЛицаПартнера(Партнер, МассивКонтактныхЛиц)
	...
	Для каждого КонтактJSON Из  МассивКонтактныхЛиц Цикл
		...
		Для каждого Поле Из КонтактJSON Цикл
			...
			Если Поле.Ключ = "name" И ОбъектКЛ.Наименование <> Поле.Значение Тогда
				ОбъектКЛ.Наименование = СокрЛП(Поле.Значение);
			ИначеЕсли  Поле.Ключ = "midname" И ЗначениеЗаполнено(Поле.Ключ)Тогда 
				ОбъектКЛ.Наименование = ОбъектКЛ.Наименование +" " + Поле.Значение;
			ИначеЕсли  Поле.Ключ = "lastname" И ЗначениеЗаполнено(Поле.Ключ)Тогда 
				ОбъектКЛ.Наименование = ОбъектКЛ.Наименование +" " + Поле.Значение;
			ИначеЕсли Поле.Ключ = "sex" Тогда
				Если КонтактJSON.sex = "Male" Тогда 
...
Функция ПолучитьСтруктуруМетаданныхДляОбмена(СлужебныеДанные) Экспорт
	...
	Если СлужебныеДанные.ИмяКонфигурации = "УправлениеТорговлей" 
		...
		// Контактные лица как тодельная сущность
