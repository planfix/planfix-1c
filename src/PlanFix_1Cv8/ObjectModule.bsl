...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	РегистрационныеДанные.Вставить("Версия", "1.3.15");
...
Функция ОтправитьДанныеВПланФиксВФоне()Экспорт
	...
	Попытка 
		...
		ЗаписатьСобытиеВЖурналРегистрации(НСтр("ru = 'Интеграция с ПланФикс. Выгрузка данных'", ОбщегоНазначения.КодОсновногоЯзыка()), 
...
Функция ПолучитьДанныеИзПланФиксВФоне(ТолькоИзмененные = Истина)Экспорт
	...
	Попытка 
		...
		ЗаписатьСобытиеВЖурналРегистрации(НСтр("ru = 'Интеграция с ПланФикс. Загрузка данных " + МетодПолученияДанных + "'", ОбщегоНазначения.КодОсновногоЯзыка()), 
...
Функция ПолучитьДанные(ИмяМетода, Смещение, РазмерПорции, ДопПараметрыЗапроса, РезультатВыполнения)
	...
	РезультатЗапроса = ВыполнитьHTTPЗапрос(ИмяМетода, "offset=" + Формат(Смещение,"ЧГ=") + "&pageSize=" + РазмерПорции + ДопПараметрыЗапроса);
...
Функция ПолучитьСсылкуНаОбъектПоEmail(МенеджерОбъекта, АдресЭлектроннойПочты, Владелец)
	...
	Если Владелец <> Неопределено Тогда
		Запрос.Текс = СтрЗаменить(Запрос.Текст,"&УсловиеПоВладелцу", "|ТаблицаОбъектаКИ.Ссылка.Владелец = &Владелец");
		Запрос.УстановитьПараметр("Владелец", Владелец);
	Иначе
		Запрос.Текс = СтрЗаменить(Запрос.Текст,"&УсловиеПоВладелцу", Истина);
	КонецЕсли; 
...
Процедура УстановитьЗначениеСвойствНовыйКонтрагент(НастройкаСопоставленияДанных, НовыйКонтрагент, КонтактJSON)
	...
	Если ИнформацияПоКонтактнымЛицам <> Неопределено И ИнформацияПоКонтактнымЛицам.Количество() > 0 Тогда
		...
		СоздатьИзменитьКонтактныеЛицаКомпании(НастройкаСопоставленияДанных, НовыйКонтрагент, ИнформацияПоКонтактнымЛицам);
	...
	ИначеЕсли НовыйКонтрагент.ВидКонтрагента <> Перечисления.ВидыКонтрагентов.ЮридическоеЛицо  Тогда 
		СоздатьИзменитьКонтактноеЛицоИзКонтрагента(НастройкаСопоставленияДанных, НовыйКонтрагент, КонтактJSON);
	КонецЕсли; 
КонецПроцедуры
...
Процедура УстановитьЗначениеСвойствТекущийКонтрагент(НастройкаСопоставленияДанных, Контрагент, КонтактJSON)
	...
	Если ИнформацияПоКонтактнымЛицам <> Неопределено И ИнформацияПоКонтактнымЛицам.Количество() > 0 Тогда
		// Не создаем контрагентов для контактов компаний
		СоздатьИзменитьКонтактныеЛицаКомпании(НастройкаСопоставленияДанных, Контрагент, ИнформацияПоКонтактнымЛицам);
		
	// Контактное лицо из контрагента создаем только для контактов	
	ИначеЕсли Контрагент.ВидКонтрагента <> Перечисления.ВидыКонтрагентов.ЮридическоеЛицо  Тогда 
		СоздатьИзменитьКонтактноеЛицоИзКонтрагента(НастройкаСопоставленияДанных, Контрагент, КонтактJSON);
	КонецЕсли; 
КонецПроцедуры
...
Процедура СоздатьИзменитьКонтактныеЛицаКомпании(НастройкаСопоставленияДанных, КонтрагентКомпания, МассивКонтактныхЛиц)
	...
	Для каждого КонтактJSON Из  МассивКонтактныхЛиц Цикл
		...
		// Внесение изменений
		УстановитьЗначениеСвойствКонтактноеЛицо(НастройкаСопоставленияДанных, КонтактноеЛицоОбъект, КонтактJSON);
		...
		УстановитьКонтактнуюИнформациюКонтактногоЛица(НастройкаСопоставленияДанных, КонтактноеЛицоОбъект, КонтактJSON);
...
Процедура УстановитьЗначениеСвойствКонтактноеЛицо(НастройкаСопоставленияДанных, КонтактноеЛицо, КонтактJSON)
	...
	УстановитьКонтактнуюИнформациюКонтактногоЛица(НастройкаСопоставленияДанных, КонтактноеЛицо, КонтактJSON);
	
	КонтактноеЛицо.Наименование = ПолучитьНаименованиеОбъектаИзJSON(КонтактJSON);
	КонтактноеЛицо.Записать();
КонецПроцедуры

Процедура УстановитьКонтактнуюИнформациюКонтактногоЛица(НастройкаСопоставленияДанных, КонтактноеЛицо, КонтактJSON)
...
// Пока не используем
Процедура СоздатьИзменитьКонтрагентовИКонтактныеЛицаКомпании(НастройкаСопоставленияДанных, КонтрагентКомпания, МассивКонтактныхЛиц)
	...
	Для каждого КонтактJSON Из  МассивКонтактныхЛиц Цикл
		...
		УстановитьКонтактнуюИнформациюКонтактногоЛицаИзКонтрагента(НастройкаСопоставленияДанных, КонтактJSON, КонтрагентДляКЛ, КонтактноеЛицоОбъект);
...
Процедура СоздатьИзменитьКонтактноеЛицоИзКонтрагента(НастройкаСопоставленияДанных, КонтрагентОбъект, КонтактJSON)
	...
	УстановитьКонтактнуюИнформациюКонтактногоЛицаИзКонтрагента(НастройкаСопоставленияДанных, КонтактJSON, КонтрагентОбъект.Ссылка, КонтактноеЛицоОбъект);
...
Процедура УстановитьКонтактнуюИнформациюКонтактногоЛицаИзКонтрагента(НастройкаСопоставленияДанных, КонтактJSON, Контрагент, КонтактноеЛицоОбъект)
...
Процедура ЗаписатьСобытиеВЖурналРегистрации(ИмяСобытия = Неопределено, ТекстСообщения, УровеньЖурнала = Неопределено)
	Если ИмяСобытия = Неопределено Тогда
		ИмяСобытия = НСтр("ru = 'Интеграция с ПланФикс. Обмен данными'", ОбщегоНазначения.КодОсновногоЯзыка());	
	КонецЕсли;
