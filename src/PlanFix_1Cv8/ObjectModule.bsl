...
Функция СведенияОВнешнейОбработке() Экспорт
	...
	РегистрационныеДанные.Вставить("Версия", "1.7.6");
...
Процедура ВыполнитьОбменДанными(РезультатВыполнения) 
	...
	СтруктураМетаданных = СлужебныеДанные.СтруктураМетаданных;
...
Функция ПолучитьДанныеИзPlanfixВФоне(ТолькоИзмененные = Истина)Экспорт
	...
	Попытка 
		...
		ЗаписатьСобытиеВЖурналРегистрации(НСтр("ru = 'Интеграция с Planfix. Загрузка данных " + МетодПолученияДанных + "'"), 
			...
			УровеньЖурналаРегистрации.Информация);
		Исключение
			ТекстСообщения  = НСтр("ru = 'Во время выполнения действия произошла неизвестная ошибка.'") + ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			
			РезультатВыполнения.РезультатЗагрузки.Результат = Ложь;
			РезультатВыполнения.ОписаниеОшибкиЗагрузки = ТекстСообщения;
...
Процедура ПолучитьДанныеИзPlanfix(ТолькоИзмененные, РезультатВыполнения)
	...
	Если СтруктураМетаданных.Количество() = 0 Тогда
		...
		ОтправитьЛог(РезультатВыполнения.ОписаниеОшибки, "warning");
		Возврат;
	КонецЕсли; 
...
Процедура КонтактыОпределитьИЗаполнитьОбщиеСвойстваОбъектаJSON(СлужебныеДанные, ЗаписьJSON, СтрокаТЗ, КолонкаТЗ, НастройкаСопоставленияДанных)
	...
	Если СлужебныеДанные.ЭтоУТКА И КолонкаТЗ.Имя = "Партнер" 
		И (ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровИКонтрагентов") ИЛИ ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровКакКонтрагентов")) 
		И ПартнерКонтрагентаИКонтрагентЭтаОднаСущность(СтрокаТЗ.Ссылка) Тогда
	...
	Для каждого ЭлементДанныхJSON Из СтруктураДанныхJSON Цикл
		...
		Если ЭлементДанныхJSON.Ключ = "sourceId" Тогда
			Если ЭлементДанныхJSON.Значение <> СокрЛП(СлужебныеДанные.ИдентификаторИБ) Тогда
				...
				Сообщить(РезультатВыполнения.ОписаниеОшибкиЗагрузки, СтатусСообщения.ОченьВажное);
				ЗаписатьСобытиеВЖурналРегистрации( ,РезультатВыполнения.ОписаниеОшибкиЗагрузки, УровеньЖурналаРегистрации.Ошибка);
				ОтправитьЛог(РезультатВыполнения.ОписаниеОшибкиЗагрузки, "warning");
				Возврат РезультатВыполнения;	
			КонецЕсли; 
			Продолжить;
		КонецЕсли; 
		...
		Для каждого ОбъектJSON Из ЭлементДанныхJSON.Значение Цикл
			...
			Если ТекущийОбъект = Неопределено Тогда
				Если НастройкаСопоставленияДанных.ОбновлятьОбъектыВ1С Тогда
					ЗаписьJSON = Новый ЗаписьJSON;
					ПараметрыЗаписиJSON = Новый ПараметрыЗаписиJSON(, Символы.Таб);
					ЗаписьJSON.УстановитьСтроку(ПараметрыЗаписиJSON);
					ЗаписатьJSON(ЗаписьJSON , ОбъектJSON);
					ТекстОшибки = НСтр("ru='Ошибка! Не удалось найти объект для записи: " + ЗаписьJSON.Закрыть() + "'");
					...
					ОтправитьЛог(ТекстОшибки, "warning");
				КонецЕсли; 
...
Функция ПолучитьОбъект(НастройкаСопоставленияДанных, МенеджерОбъекта, ОбъектJSON, Владелец = Неопределено)
	...
	Если ОбъектСсылка = Неопределено ИЛИ Не ЗначениеЗаполнено(ОбъектСсылка.ВерсияДанных) Тогда
...
Функция ПолучитьСсылкуНаОбъектПоДвумПользовательскимПолям(НастройкаСопоставленияДанных, МенеджерОбъекта, ОбъектJSON)
	...
	СлужебныеДанные = ПолучитьСлужебныеДанные();
	СтруктураОбъектаМетаданных = СлужебныеДанные.СтруктураМетаданных;
...
Функция ПолучитьСсылкуНаОбъектПоEmail(МенеджерОбъекта, АдресЭлектроннойПочты, Владелец)
	...
	ИмяТаблицыБд = СтруктураОбъектаМетаданных.contacts.ИмяТаблицыБд;
	Если ОбъектСсылка.Метаданные().Имя = "КонтактныеЛицаПартнеров"  Тогда 
		ИмяТаблицыБд = СтруктураОбъектаМетаданных.contact_persons.ИмяТаблицыБд;	
	КонецЕсли;
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	" + ИмяТаблицыБд + ".КонтактнаяИнформация КАК ТаблицаОбъектаКИ
		|ГДЕ
...
Процедура УстановитьЗначениеСвойствОбъекта(СлужебныеДанные, НастройкаСопоставленияДанных, ТекущийОбъект, ОбъектJSON)
	...
	Если СлужебныеДанные.ЭтоУТКА И ТекущийОбъект.Метаданные().Имя = "Контрагенты" 
				И(ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровИКонтрагентов") ИЛИ ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровКакКонтрагентов")) Тогда
...
Функция  УстановитьЗначениеСвойствНовыйПартнер(НастройкаСопоставленияДанных, НовыйПартнер, КонтактJSON)
	...
	Для каждого Поле Из КонтактJSON Цикл
		...
		ИначеЕсли Поле.Ключ = "group" И Поле.Значение.Свойство("name") Тогда
			ИмяГруппы = Поле.Значение.name;
	...
	ИспользоватьПартнеровКакКонтрагентовУстановитьЗначениеКонтрагента(НовыйПартнер.Ссылка);	
...
Процедура УстановитьЗначениеСвойствТекущийПартнер(НастройкаСопоставленияДанных, Партнер, КонтактJSON)
	Если  Не Партнер.Клиент И Не Партнер.Поставщик И Не Партнер.Конкурент И Не Партнер.ПрочиеОтношения Тогда
	...
	ИспользоватьПартнеровКакКонтрагентовУстановитьЗначениеКонтрагента(Партнер.Ссылка);	
...
Процедура ИспользоватьПартнеровКакКонтрагентовУстановитьЗначениеКонтрагента(ПартнерСсылка)
	
	Если ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровКакКонтрагентов") Тогда
		КонтрагентПартнера = Справочники.Контрагенты.НайтиПоРеквизиту("Партнер", ПартнерСсылка.Ссылка);
		...
		КонтрагентОбъект.Наименование = ПартнерСсылка.Наименование;
		КонтрагентОбъект.НаименованиеПолное        = ПартнерСсылка.НаименованиеПолное;
		КонтрагентОбъект.Партнер                   = ПартнерСсылка.Ссылка;
		КонтрагентОбъект.ЮрФизЛицо                 = ПартнерСсылка.ЮрФизЛицо;
		КонтрагентОбъект.ДополнительнаяИнформация  = ПартнерСсылка.ДополнительнаяИнформация;
		
		ПсевдоФорма = Новый Структура();
		ПсевдоФорма.Вставить("ЮридическийАдресКонтрагентаПредставление","");
		ПсевдоФорма.Вставить("ЮридическийАдресКонтрагента","");
		
		МодульПартнерыИКонтрагенты = ОбщегоНазначения.ОбщийМодуль("ПартнерыИКонтрагенты");
		МодульПартнерыИКонтрагенты.ЗаполнитьКонтактнуюИнформациюКонтрагентаПартнера(КонтрагентОбъект, ПсевдоФорма, ПартнерСсылка.КонтактнаяИнформация);
...
Процедура СоздатьИзменитьКонтактныеЛицаКомпанииУНФ(НастройкаСопоставленияДанных, КонтрагентКомпания, МассивКонтактныхЛиц)
	...
	Для каждого КонтактJSON Из  МассивКонтактныхЛиц Цикл
		...
		// Запись объекта
		КонтактноеЛицоОбъект.ДополнительныеСвойства.Вставить("СвязьСКонтрагентом", Новый Структура("Контрагент,Должность,Порядок", КонтрагентКомпания.Ссылка, "", 0));
...
Функция ПолучитьЗначениеПоля(ДанныеПоля)
	...
	ИначеЕсли ДанныеПоля.field.type = 1 Тогда //"Число"
		...
		Попытка
			Возврат Формат(Число(ЗначениеПоля),"ЧГ=0" );	
		Исключение
			Возврат Неопределено;
		КонецПопытки; 
...
Функция ВыполнитьHTTPЗапрос(ИмяМетода, ТелоЗапроса = "", HTTPМетод = "POST", Таймаут = 60) Экспорт
	Если ИмяМетода <> "debug" И ИмяМетода <> "ping" Тогда
		ОтправитьЛог();	
	КонецЕсли; 
	...
	Если СтрНачинаетсяС(ТелоЗапроса, "{") Тогда
...
Функция ПолучитьСлужебныеДанные() Экспорт
	...
	СлужебныеДанные.Вставить("ИнформацияОбОшибке", 		"");
	...
	СлужебныеДанные.Вставить("ИдентификаторИБ"	    	, Строка(ПолучитьУникальныйИдентификаторИБ()));
	...
	Если Метаданные.Имя = "УправлениеТорговлей" 
		...
		//Информбокс https://informbox.ru +
		ИЛИ Метаданные.Имя = "УправлениеТорговлей_CRM_2"		
		//Информбокс https://informbox.ru -
	...
	СлужебныеДанные.Вставить("СтруктураМетаданных", ПолучитьСтруктуруМетаданныхДляОбмена(СлужебныеДанные));
	
	НастройкаСопоставленияДанныхКонтактов = ПолучитьНастройкуСопоставленияДанных("contacts");
	НастройкаСопоставленияДанныхКонтактов.Вставить("ТаблицаСопоставленияПользовательскихПолейИРеквизитов", ТаблицаЗначенийВМассив(НастройкаСопоставленияДанныхКонтактов.ТаблицаСопоставленияПользовательскихПолейИРеквизитов));
	НастройкаСопоставленияДанныхКонтактов.Вставить("ТаблицаСопоставленияВидовНомераТелефона", ТаблицаЗначенийВМассив(НастройкаСопоставленияДанныхКонтактов.ТаблицаСопоставленияВидовНомераТелефона));
	
	СлужебныеДанные.Вставить("НастройкаСопоставленияДанныхКонтактов", НастройкаСопоставленияДанныхКонтактов);
...
Функция ПолучитьСтруктуруМетаданныхДляОбмена(СлужебныеДанные) Экспорт
	...
	Если СлужебныеДанные.ЭтоУТКА Тогда
		...
		Если ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровИКонтрагентов")ИЛИ 
			ПолучитьФункциональнуюОпцию("ИспользоватьПартнеровКакКонтрагентов")Тогда
...
Процедура ОтправитьЛог(ИнформацияОбОшибке = "", Уровень = "info")
	
	НастройкиПодключения  = ПолучитьНастройкиПодключения();
	Если НастройкиПодключения <> Неопределено Тогда
			РежимОтладки = Ложь;
		Если НастройкиПодключения.Свойство("РежимОтладки") Тогда
			РежимОтладки = НастройкиПодключения.РежимОтладки; 
		КонецЕсли; 
		
		Если РежимОтладки Тогда
			ПараметрыЗаписиJSON = Новый ПараметрыЗаписиJSON(, Символы.Таб);
			
			СлужебныеДанныеJSON = Новый ЗаписьJSON;
			СлужебныеДанныеJSON.УстановитьСтроку(ПараметрыЗаписиJSON);
			СД = ПолучитьСлужебныеДанные();
			СД.ИнформацияОбОшибке = ИнформацияОбОшибке;
			
			
			Попытка
				ЗаписатьJSON(СлужебныеДанныеJSON , СД);	
			Исключение
				СД = Новый Структура();
				СД.Вставить("ИнформацияОбОшибке", 	ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
				ЗаписатьJSON(СлужебныеДанныеJSON , СД);	
			КонецПопытки; 
			...
			ЗаписьJSON.ЗаписатьНачалоОбъекта();
				ЗаписьJSON.ЗаписатьИмяСвойства("level");
				ЗаписьJSON.ЗаписатьЗначение(Уровень);
				
				ЗаписьJSON.ЗаписатьИмяСвойства("msg");
				ЗаписьJSON.ЗаписатьЗначение(СлужебныеДанныеJSON.Закрыть());
			ЗаписьJSON.ЗаписатьКонецОбъекта();
			
			ВыполнитьHTTPЗапрос("debug", ЗаписьJSON.Закрыть());
		КонецЕсли; 
