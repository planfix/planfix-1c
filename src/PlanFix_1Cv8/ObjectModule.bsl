...
Функция СофрмироватьJSON(ТаблицаВыгружаемыхДанных, ИмяОбъектаПланфикс, ИдентификаторИБ, НастройкаСопоставленияДанных) Экспорт
	...
	Для каждого СтрокаТЗ Из ТаблицаВыгружаемыхДанных Цикл
		...
		Для каждого КолонкаТЗ Из ТаблицаВыгружаемыхДанных.Колонки Цикл
			...
			ОпределитьИЗаполнитьОбщиеСвойстваОбъектаJSON(ЗаписьJSON, СтрокаТЗ, КолонкаТЗ);
...
Процедура ОпределитьИЗаполнитьОбщиеСвойстваОбъектаJSON(ЗаписьJSON, СтрокаТЗ, КолонкаТЗ)
	...
	КонтактыОпределитьИЗаполнитьОбщиеСвойстваОбъектаJSON(ЗаписьJSON, СтрокаТЗ, КолонкаТЗ);
...
Процедура КонтактыОпределитьИЗаполнитьОбщиеСвойстваОбъектаJSON(ЗаписьJSON, СтрокаТЗ, КолонкаТЗ)
...
Процедура УстановитьЗначениеСвойствОбъекта(НастройкаСопоставленияДанных, ТекущийОбъект, ОбъектJSON)
	...
	Если ТекущийОбъект.Метаданные().Имя = "Контрагенты" 
		...
		Если ТекущийОбъект.Ссылка.Пустая()Тогда
			 УстановитьЗначениеСвойствНовыйКонтрагентУТКА(ТекущийОбъект, ОбъектJSON);
		Иначе
			 УстановитьЗначениеСвойствТекущийКонтрагентУТКА(ТекущийОбъект, ОбъектJSON);
		КонецЕсли;
	ИначеЕсли ТекущийОбъект.Метаданные().Имя = "Контрагенты" Тогда 
		
		Если ТекущийОбъект.Ссылка.Пустая() Тогда
			 УстановитьЗначениеСвойствНовыйКонтрагент(ТекущийОбъект, ОбъектJSON);
		Иначе
			 УстановитьЗначениеСвойствТекущийКонтрагент(ТекущийОбъект, ОбъектJSON);
		КонецЕсли; 
	ИначеЕсли ТекущийОбъект.Метаданные().Имя = "Партнеры" Тогда 
		
		Если ТекущийОбъект.Ссылка.Пустая()Тогда
			 УстановитьЗначениеСвойствНовыйПартнер(ТекущийОбъект, ОбъектJSON);
		Иначе
			 УстановитьЗначениеСвойствТекущийПартнер(ТекущийОбъект, ОбъектJSON);
		КонецЕсли;
	КонецЕсли; 
...
Процедура УстановитьЗначениеСвойствНовыйКонтрагентУТКА(НовыйКонтрагент, КонтактJSON)
	...
	УстановитьКонтактнуюИнформациюКонтрагентаУТКА(НовыйКонтрагент, КонтактJSON) ;
	...
	НовыйКонтрагент.Партнер =  УстановитьЗначениеСвойствНовыйПартнер(Справочники.Партнеры.СоздатьЭлемент(), КонтактJSON);
	НовыйКонтрагент.Записать();
	
КонецПроцедуры

Процедура УстановитьЗначениеСвойствТекущийКонтрагентУТКА(Контрагент, КонтактJSON)
	...
	КонецЦикла;
	УстановитьКонтактнуюИнформациюКонтрагентаУТКА(Контрагент, КонтактJSON); 
...
Процедура УстановитьКонтактнуюИнформациюКонтрагентаУТКА(Контрагент, КонтактJSON) 
...
Функция  УстановитьЗначениеСвойствНовыйПартнер(НовыйПартнер, КонтактJSON)
	...
	КонецЦикла;
	УстановитьКонтактнуюИнформациюПартнера(НовыйПартнер, КонтактJSON);
	...
	Если ИнформацияПоКонтактнымЛицам <> Неопределено Тогда
		СоздатьИзменитьКонтактныеЛицаПартнера(НовыйПартнер, ИнформацияПоКонтактнымЛицам);
	КонецЕсли; 
...
Процедура УстановитьЗначениеСвойствТекущийПартнер(Партнер, КонтактJSON)
	...
	Для каждого Поле Из КонтактJSON Цикл
		...
		ИначеЕсли Поле.Ключ = "contacts" Тогда
			СоздатьИзменитьКонтактныеЛицаПартнера(Партнер, Поле.Значение);
		КонецЕсли;
	КонецЦикла;
	УстановитьКонтактнуюИнформациюПартнера(Партнер, КонтактJSON);
...
Процедура УстановитьКонтактнуюИнформациюПартнера(Партнер, КонтактJSON)
...
Процедура СоздатьИзменитьКонтактныеЛицаПартнера(Партнер, МассивКонтактныхЛиц)
	...
	Для каждого КонтактJSON Из  МассивКонтактныхЛиц Цикл
		...
		УстановитьКонтактнуюИнформациюКЛ (ОбъектКЛ, КонтактJSON);
...
Процедура УстановитьКонтактнуюИнформациюКЛ(ОбъектКЛ, КонтактJSON)
	...
	Для каждого Поле Из КонтактJSON Цикл
		...
		ИначеЕсли Поле.Ключ = "phones" Тогда
			Для каждого СтрокаТелефон Из Поле.Значение Цикл
				Отбор = Новый Структура();
				Отбор.Вставить("Тип",Перечисления.ТипыКонтактнойИнформации.Телефон);
				Отбор.Вставить("Вид",Справочники.ВидыКонтактнойИнформации.ТелефонКонтактногоЛица);
				Отбор.Вставить("НомерТелефона",НормализоватьНомерТелефона(СтрокаТелефон.number));
...
Процедура УстановитьЗначениеСвойствНовыйКонтрагент(НовыйКонтрагент, КонтактJSON)
	// Значения по умолчанию
	...
	УстановитьКонтактнуюИнформациюКонтрагента(НовыйКонтрагент, КонтактJSON);
...
Процедура УстановитьЗначениеСвойствТекущийКонтрагент(Контрагент, КонтактJSON)
	...
	УстановитьКонтактнуюИнформациюКонтрагента(Контрагент, КонтактJSON);
...
Процедура УстановитьКонтактнуюИнформациюКонтрагента(Контрагент, КонтактJSON)
...
Процедура СоздатьКонтактноеЛицо(Контрагент)
	...
	УстановитьКонтактнуюИнформациюКонтактногоЛица(Контрагент, КонтактноеЛицоОбъект);
...
Процедура УстановитьКонтактнуюИнформациюКонтактногоЛица(Контрагент, КонтактноеЛицоОбъект)
