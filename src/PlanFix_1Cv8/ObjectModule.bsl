	...
	Если КолонкаТЗ.Имя = "КонтактнаяИнформация" И ТипЗнч(СтрокаТЗ[КолонкаТЗ.Имя]) = Тип("ТаблицаЗначений")  Тогда
		...
		Если МассивПоиска.Количество() > 0 Тогда
			...
			Для каждого СтрокаТелефон Из МассивПоиска  Цикл
				...
				ЗаписьJSON.ЗаписатьЗначение(НормализоватьНомерТелефона(СтрокаТелефон.Представление));
...
Функция ПолучитьОбъект(НастройкаСопоставленияДанных, МенеджерОбъекта, ОбъектJSON)
	...
	Если ОбъектJSON.Свойство("sourceObjectId") И ЗначениеЗаполнено(ОбъектJSON.sourceObjectId) Тогда
		...
		КонецЕсли; 
		GUID = Новый УникальныйИдентификатор(sourceObjectId);
	...
	Для каждого Поле Из КонтактJSON Цикл
		...
		ИначеЕсли Поле.Ключ = "phones" Тогда
			Для каждого СтрокаТелефон Из Поле.Значение Цикл
				Вид = ПолучитьВидНомераТелефона(НастройкаСопоставленияДанных, СтрокаТелефон.type, Контрагент.Ссылка);
				...
				МассивИскомыхСтрок = Контрагент.КонтактнаяИнформация.НайтиСтроки(Отбор);
				Если МассивИскомыхСтрок.Количество() = 0 И СокрЛП(СтрокаТелефон.number) <> "" Тогда
					СтрокаКИ = Контрагент.КонтактнаяИнформация.Добавить(); 
...
Процедура УстановитьКонтактнуюИнформациюПартнера(НастройкаСопоставленияДанных, Партнер, КонтактJSON)
	...
	Для каждого Поле Из КонтактJSON Цикл
		...
		Если Поле.Ключ = "email" Тогда
			...
			ИначеЕсли Поле.Ключ = "phones" Тогда
				Для каждого СтрокаТелефон Из Поле.Значение Цикл
					Вид = ПолучитьВидНомераТелефона(НастройкаСопоставленияДанных, СтрокаТелефон.type, Партнер.Ссылка);
...
Процедура СоздатьИзменитьКонтактныеЛицаПартнера(НастройкаСопоставленияДанных,Партнер, МассивКонтактныхЛиц)
	...
	Для каждого КонтактJSON Из  МассивКонтактныхЛиц Цикл
		// пока ищем по наименованию
		Отбор = Новый Структура();
		Отбор.Вставить("Наименование", КонтактJSON.name + " " + КонтактJSON.midname + " " + КонтактJSON.lastname);
...
Процедура УстановитьКонтактнуюИнформациюКЛ(НастройкаСопоставленияДанных,ОбъектКЛ, КонтактJSON)
	...
	Для каждого Поле Из КонтактJSON Цикл
		...
		ИначеЕсли Поле.Ключ = "phones" Тогда
			Для каждого СтрокаТелефон Из Поле.Значение Цикл
				Вид = ПолучитьВидНомераТелефона(НастройкаСопоставленияДанных, СтрокаТелефон.type, ОбъектКЛ.Ссылка);
...
Процедура УстановитьЗначениеСвойствНовыйКонтрагент(НастройкаСопоставленияДанных, НовыйКонтрагент, КонтактJSON)
	...
	СоздатьКонтактноеЛицо(НастройкаСопоставленияДанных, КонтактJSON, НовыйКонтрагент.Ссылка);
...
Процедура УстановитьЗначениеСвойствТекущийКонтрагент(НастройкаСопоставленияДанных, Контрагент, КонтактJSON)
	...
	Если Контрагент.КонтактноеЛицо.Пустая()Тогда
		СоздатьКонтактноеЛицо(НастройкаСопоставленияДанных, КонтактJSON, Контрагент.Ссылка);
	Иначе 
		ИзменитьКонтактноеЛицо(НастройкаСопоставленияДанных, КонтактJSON, Контрагент.КонтактноеЛицо, Контрагент.Ссылка);
	КонецЕсли; 
		...
		ИначеЕсли Поле.Ключ = "phones"  Тогда
			Для каждого СтрокаТелефон Из Поле.Значение Цикл
				...
				Если МассивИскомыхСтрок.Количество() = 0 И СокрЛП(СтрокаТелефон.number) <> "" Тогда
					...
					СтрокаКИ.Вид = Вид;
					СтрокаКИ.НомерТелефона = НормализоватьНомерТелефона(СтрокаТелефон.number);
					СтрокаКИ.Представление = НормализоватьНомерТелефона(СтрокаТелефон.number);
					СтрокаКИ.Значение = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияПоПредставлению(СтрокаКИ.Представление, СтрокаКИ.Тип);
					СтрокаКИ.ЗначенияПолей = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияВXML(СтрокаКИ.Значение);
				КонецЕсли; 
...
Процедура СоздатьКонтактноеЛицо(НастройкаСопоставленияДанных, КонтактJSON, Контрагент)
...
Процедура ИзменитьКонтактноеЛицо(НастройкаСопоставленияДанных, КонтактJSON, КонтактСсылка, Контрагент)
...
Процедура УстановитьКонтактнуюИнформациюКонтактногоЛица(НастройкаСопоставленияДанных, КонтактJSON, Контрагент, КонтактноеЛицоОбъект)
	...
	Для каждого Поле Из КонтактJSON Цикл
		Если Поле.Ключ = "phones" Тогда
			Для каждого СтрокаТелефон Из Поле.Значение Цикл
				Вид = ПолучитьВидНомераТелефона(НастройкаСопоставленияДанных, СтрокаТелефон.type, КонтактноеЛицоОбъект.Ссылка);
				...
				МассивИскомыхСтрок = КонтактноеЛицоОбъект.КонтактнаяИнформация.НайтиСтроки(Отбор);
				Если МассивИскомыхСтрок.Количество() = 0 И СокрЛП(СтрокаТелефон.number) <> "" Тогда
					СтрокаКИ = КонтактноеЛицоОбъект.КонтактнаяИнформация.Добавить(); 
					СтрокаКИ.Тип = Перечисления.ТипыКонтактнойИнформации.Телефон;
					СтрокаКИ.Вид = Вид;
					СтрокаКИ.НомерТелефона = НормализоватьНомерТелефона(СтрокаТелефон.number);
					СтрокаКИ.Представление = СтрокаТелефон.number;
...
Функция ПолучитьНастройкуСопоставленияДанных(ИмяЗагружаемогоОбъектаПанфикс) Экспорт
	...
	Если ИмяКлючаНастроек = "contacts" Тогда
		...
		// контактные лица (контакты компаний в Планфикс)
		...
		Иначе
			СтруктураНастройкиОбъекта.Вставить("ЗагружатьОбновлятьКонтактныеЛицаИзКомпаний", "");
		КонецЕсли;
	КонецЕсли;
...
Функция ПолучитьВидНомераТелефона(НастройкаСопоставленияДанных, ИдентификаторТипаПФ, ОбъектСсылка)
	...
	СтрокаТаблицы = НастройкаСопоставленияДанных.ТаблицаСопоставленияВидовНомераТелефона.Найти(ИдентификаторТипаПФ, "ВидНомераТелефонаПФ");
	
	Если СтрокаТаблицы <> Неопределено Тогда
		ВидНомераТелефона = СтрокаТаблицы.ВидНомераТелефонаКИ;
	Иначе 	
		// Пробуем найти в предопределенных
		Если ОбъектСсылка.Метаданные().Имя = "Контрагенты" Тогда
			ВидНомераТелефона = Справочники.ВидыКонтактнойИнформации.ТелефонКонтрагента;
		ИначеЕсли ОбъектСсылка.Метаданные().Имя = "Партнеры" Тогда
			ВидНомераТелефона = Справочники.ВидыКонтактнойИнформации.ТелефонПартнера;
		ИначеЕсли ОбъектСсылка.Метаданные().Имя = "КонтактныеЛицаПартнеров" Тогда
			Если ИдентификаторТипаПФ = 1 Тогда	
				ВидНомераТелефона = Справочники.ВидыКонтактнойИнформации.МобильныйТелефонКонтактногоЛица;	
			ИначеЕсли ИдентификаторТипаПФ = 2 Тогда 
				ВидНомераТелефона = Справочники.ВидыКонтактнойИнформации.ТелефонКонтактногоЛица;	
			КонецЕсли;
